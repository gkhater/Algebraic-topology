...
\begin{proof}
    Let $X = D^k$ and $A = \partial D^k = S^{k-1}$, this is a good pair because $D^k - \set{0}$ deformation retracts to $S^{k-1}$. 
    Also $X / A \cong S^k$, so 
    $$\cdots \to \tilde{H}_{n} S^{k-1} \to \tilde{H}_n(D^k)  = 0\to \tilde{H}_n(S^k) \to \tilde{H}_{n-1} (S^{k-1}) \to \tilde{H}_{n-1} (D^{k}) = 0 \to \cdots$$
    So we get that 
    $$\tilde{H}_n(S^k) \cong \tilde{H}_{n-1} (S^{k-1})$$
    We have three cases: 
    \begin{itemize}
        \item \textbf{Case 1. $n > k$} we have 
        $$\tilde{H}_n(S^{k-1}) \cong \tilde{H}_{n-k} (S^0) \cong 0$$

        \item \textbf{Case 2. $n < k$} We get 
        $$\tilde{H}_n(S^k) \cong \tilde{H}_0 (S^{k-n}) \cong 0$$

        \item \textbf{Case 3. $n = k$} We get 
        $$\tilde{H}_n(S^k) \cong \tilde{H}_0 (S^0)$$
    \end{itemize}
\end{proof}
