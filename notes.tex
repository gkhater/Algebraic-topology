...
\begin{definition}
    Let $S$ be a set, then the free abelian group over $S$ is an abelian group denoted by $F(S)$ together with a map $\iota \colon S \to F(S)$ 
    s.t for any map $\chi \colon i \colon S \to A$ ($A$ abelian) there is a unique homomorphism $\varphi \colon F(S) \to A$ s.t the following diagram commutes
    $$ $$  
    It is easy to see that $j$ must be injective. 
\end{definition}
...
\begin{example}
    ...
    indeed, $\varphi(j(x)) = a = \xi (x)$ abd $\varphi$ is unique since it $\varphi'(1) = a$ and $1$ generates $\Z$. 
...
S_\alpha
  ...
  \arrow[drr,bend right=18,"\chi_\alpha"']
&
F(S_\alpha)
  \arrow[r,"\iota_\alpha"]
  \arrow[dr,"\varphi_\alpha"']
&
\displaystyle\prod_{\beta}^{*} F(S_\beta)
  \arrow[d,dashed,"\Phi"]
\\
S
  \arrow[ur,bend right=15,"j"']
  \arrow[rr,bend left=10,"\chi"]
&&
A
\end{tikzcd}
\]
...
\]
\item Associativity follows from associativity of concatenation and
uniqueness of reduction to reduced form.
\end{itemize}

Thus reduced words form a group, which satisfies the universal
mapping property of the free product.
\end{proof}
